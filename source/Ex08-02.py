...
def loadImage(fname):  # raw --> DB
    ...
    con = sqlite3.connect("C:/CookAnalysis/rawDB")  # DB가 저장된 폴더까지 지정
    cur = con.cursor()
    fp = open(fname, 'rb')
...
def loadDatabase():  # DB --> 메모리
    ...
    con = sqlite3.connect("C:/CookAnalysis/rawDB")
    cur = con.cursor()
...
if __name__ == "__main__":
    ...
    # 테이블 초기화
    con = sqlite3.connect("C:/CookAnalysis/rawDB")  # 소스코드가 저장된 폴더에 생성됨
    cur = con.cursor()
